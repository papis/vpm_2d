...
contains
    ...
    !--------------------------------------------------------------------------!
    Subroutine project_particles_2D(Qproj,Qpar,QpX,Qprojtype,ipar,isize,ieq,neq,QINF,isizepar)
        !use pmgrid

        Implicit None
        integer,intent(in) :: ipar,isize,ieq(neq),isizepar
        double precision ,intent(out):: Qproj(isize,NXpm,NYpm,NZpm)
        double precision ,allocatable:: Qprojpriv(:,:,:,:)
        double precision ,intent(in) :: Qpar(isize,isizepar),QpX(3,isizepar),QINF(neq)
        integer,intent(in) :: Qprojtype(isizepar)
